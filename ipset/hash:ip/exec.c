...
#define SETNAME "desired set name"
...
int main(void)
{
	...
	if(f) {
		...
		while((len = getline(&line, &size, f)) > 0) {
			...
			mnl_attr_put_u32(nlh, IPSET_ATTR_IPADDR_IPV4|NLA_F_NET_BYTEORDER, 0x01010101);
